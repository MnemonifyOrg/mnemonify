...
| P1-55 | **Self-owned block visibility rule:** Any block can carry its own `visibility_condition` (the same condition shape used by trigger "if" clauses and the Continue gate) directly in its settings panel, independent of any other block's trigger. Evaluated reactively on every variable change, not just once on page load. Found missing during hands-on instructional-designer testing: authoring "show this block once a variable is true" required editing a *different* block's trigger, which is unintuitive when the logic conceptually belongs to the block being shown |
| P1-56 | **Custom block labels:** Any block can carry an optional author-set `label` string, shown in every place blocks are listed by name (trigger target pickers, move/copy-to-page pickers). Falls back to an auto-generated type+position label ("Image (3)") when unset. Found missing during hands-on testing: with several images on one page, the generic "Image" label in trigger dropdowns gave no way to tell them apart |
