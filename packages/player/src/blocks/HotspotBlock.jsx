...
export default function HotspotBlock({ block, assets, onOpenModal, onTrigger }) {
  ...
  return <div className="block hotspot"><div className="hotspot__image-wrap"><img className="hotspot__image" src={assetSrc(image)} alt={image.alt || ''} />{regions.map((region) => <button type="button" className={`hotspot__region ${found.has(region.region_id) ? 'hotspot__region--found' : ''}`} key={region.region_id} aria-label={`Hotspot ${region.region_id}`} style={{ left: `${region.x_pct}%`, top: `${region.y_pct}%`, width: `${region.width_pct}%`, height: `${region.height_pct}%` }} onClick={() => activate(region)} />)}</div>{block.content.mode === 'quiz' && <p className="hotspot__status" role="status">Found {found.size} of {regions.filter((region) => region.correct === true).length} correct regions</p>}</div>;
}
