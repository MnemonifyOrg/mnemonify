...
export function buildBackendUrl(requestUrl, backendBaseUrl) {
  ...
  if (requestPath.startsWith('/player-assets/')) {
    // The editor itself owns /assets/* in the Pages output. Render exposes
    // the player bundle at /assets/*, so this private proxy path avoids that
    // collision without requiring a second Pages project for the player.
...
export async function onRequest(context) {
  ...
  if (!isProxyPath(pathname)) return context.next();

  const backendBaseUrl = context.env?.RENDER_BACKEND_URL;
  if (!backendBaseUrl) {
    return new Response('RENDER_BACKEND_URL is not configured.', { status: 500 });
  }

  try {
    const targetUrl = buildBackendUrl(context.request.url, backendBaseUrl);
    // Cloning the incoming Request onto the Render URL preserves method,
    // body, authorization headers, and the Pages-domain session cookie.
    ...
    console.error('[pages-proxy] Render request failed:', error);
