...
| 4.5 | Foundations for Course Health (informed by ARCHITECTURE-AUDIT.md and the strategic docs) | **COMPLETE.** **4.5a Identity + migration** shipped in `2e5da013` and `04702214` (stable IDs and the sequential migration service, including migration persistence at the autosave boundary). **4.5b Block registry + dependency index** shipped in `54296374`. **4.5c Minimal technical Course Analyzer** shipped in `7cdc920d`/`5242bdda`, with follow-up fixes `893ec5c6` and `90096eca`. All three sub-phases were verified against real production course data, not only fixtures and automated tests. Hands-on verification caught and fixed the 4.5a migration-persistence bug (autosave could bypass migration), the 4.5c false-positive duplicate-ID classification bug, and genuine legacy duplicate answer-option IDs repaired by a proper migration. The scope remains limited to the existing requirements: deterministic high-confidence schema/reference, accessibility, and asset rules, the finding model, Course Health panel, and publish gating — not the larger ARCHITECTURE-AUDIT.md/COURSE-ANALYZER.md vision |
| 4.6 | UX polish pass (informed by UX-AUDIT.md, Priority 1 items only) | **PARTIALLY COMPLETE.** **Course Health finding grouping** was built as part of Phase 4.5c. Basic/Advanced settings grouping (built on the 4.5b registry so it's consistent per block); collapsible page and settings panels + Focus Mode; between-block insertion controls; simplified primary toolbar (Preview consolidated, device widths inside preview; less-frequent actions under "More tools"); clearer module/page visual hierarchy; reduced border/icon density on unselected blocks; contextual "i" info tooltips; and the **bulk alt-text review screen** reachable from a grouped alt-text finding remain separate 4.6 work. The bulk screen shows every flagged image in one focused list (thumbnail, its existing caption as read-only reference text, an editable alt-text field the author fills or adapts) instead of hunting through pages one image at a time — explicitly does NOT auto-copy caption into alt text silently, since a screen reader announces both and silent duplication makes the listening experience worse, not better; author always makes the conscious edit. **Terminology decision:** keep all existing standard terminology as the visible labels (Trigger, Variables, Completion rule, etc.) — do NOT rename — and add accessible, keyboard-operable "i" info tooltips with teaching-oriented plain-language explanations (sourced from UX-AUDIT.md section 5's explanations) on domain-specific/advanced controls only. This serves both the first-time educator and the experienced-ID validation audience without a credibility tax. Intent-based Add Content (UX-AUDIT §6), plain-language logic recipes (UX-AUDIT §14), and AI-suggested alt text (a real future idea, needs an API integration and is properly Phase 5+ scope per the project's principle of keeping AI features optional and outside the free core) are explicitly DEFERRED — they are real features, not UX polish |
| 5 | States, expanded block types, interactive video, captions, PDF artifacts, analytics, Word importers. Translation parked to P3-1, revisit post-beta | **PARTIALLY COMPLETE — status reconciled by the July 26, 2026 accuracy audit.** **ALREADY BUILT, with no feature flag:** interactive video timeline pause points/overlays (P1-22); captions/transcripts with manual VTT/SRT upload plus the local Whisper/ffmpeg pipeline (P1-23); flashcards (P1-59); matching (P1-60); ordering (P1-61); image hotspot (P1-62); and reflection (P1-46). **PARTIAL:** analytics telemetry capture, validation, and storage (P1-24) are built, but there is no dashboard, aggregate/per-learner reporting view, or CSV/Excel export yet. **COMPLETE after live bug fix:** PDF publish artifacts (P1-18) are generated and stored, and commit `1239c751` fixed the production gap where generated PDFs were reachable in editor preview but absent from the real learner `/content/:courseId` response; a real course was checked before/after and the generated PDF downloaded with HTTP 200. **NOT BUILT:** Smart Import (P1-34) as semantic document parsing — the existing Word import is template-based table-note mapping only; AI Import (P1-33); and text-to-speech (P1-47). The remaining gaps are roadmap scope, not v1 implementation work. |
| 6 | **COMPLETE** — Accounts, shared library, review and commenting, anonymous links, deployment, and the P1-69 through P1-74 authoring/reuse extensions | Phase 6a (accounts, roles, and permissions) shipped in `11d0f6ef`, with fixes `ae2f3af0` and `eb11b3f4`; Phase 6b (review and commenting) shipped in `687d11aa`; Phase 6c (anonymous share links) shipped in `5fcd7e70`. All three sub-phases were verified against real accounts and real data, not just automated tests. Hands-on testing found and fixed a missing local migration and a token-invalidation bug in 6a, plus a live PostgreSQL publish bug during 6c verification. Real server-side permission enforcement was directly confirmed with actual role-based credentials at every stage, not just through UI checks. Deployment to Vercel + Railway + R2 is the next milestone. **P1-69 through P1-74 are already implemented and shipped ahead of this formal phase sequence; they remain held out of the v1 release for phased v1.1–v1.4 rollout.** |
...
**Phase table status clarification:** Phase 4.5 (4.5a, 4.5b, and 4.5c) is **COMPLETE** as scoped in this document and was verified against real production course data. This does not expand the phase to the larger ARCHITECTURE-AUDIT.md/COURSE-ANALYZER.md vision. Phase 4.6 remains partial: its Course Health finding-grouping item was delivered through 4.5c, but Basic/Advanced settings grouping and the bulk alt-text review screen are not built. The broader UI modernization must not be read as completion of those remaining 4.6 deliverables.
**Refer to Phase 4.5a: Stable IDs and Schema Migration Service in the ARCHITECTURE.md
***Refer to Phase 4.5b: Block Registry and Dependency Index in the ARCHITECTURE.md
**Refer to Phase 4.5c: Minimal Course Analyzer in the ARCHITECTURE.md
...
These additions preserve the formal phase numbering. Phase 4.5 is complete within the narrow scope stated in its table row; the additions above do not expand it to the larger architecture-audit vision. Phase 4.6 remains partial because only its Course Health finding-grouping item shipped, while its Basic/Advanced settings grouping and bulk alt-text review screen remain future work.
...
7. **(QA, non-blocking)** The P0-B left-nav stress test at 30+ pages was never explicitly re-confirmed at that scale.
8. **(QA, non-blocking)** The question-bank import ID-regeneration fix (`cdb822b8`) remains unverified because the feature flag is currently off for v1.
**Refer to the Phase 6a: Accounts, Roles, and Permissions section in the ARCHITECTURE.md
**Refer to the Phase 6b: Review and Commenting in the ARCHITECTURE.md
***Refer to the Phase 6c: Anonymous Share Links in the ARCHITECTURE.md
